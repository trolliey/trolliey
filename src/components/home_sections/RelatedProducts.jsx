...
function RelatedProducts({ cols, no_text , category}) {
    ...
    useEffect(() => {
        dispatch(get_all_products_Action(category, page, limit))
        setLimit(5)
    ...
    return (
        <div className="items flex-col bg-white rounded md:px-8 px-4 w-full">
            <div className="md:text-lg text-sm md:py-8 py-4 flex flex-row items-center justify-between">
                <p className="font-semibold text-gray-700 capitalize ">related products</p>
            ...
            <div className="w-full">
                {
                    loading ? (
                        <div className={`${loading || error ? "flex-1 flex w-full " : `grid ${cols ? cols : "lg:grid-cols-5 "} md:grid-cols-3 grid-cols-2`}  gap-8`}>
                            {
